...
We use both z-test and likelihood ratio test (LRT) to test the second variance component in the LMM with two-components random effects. Since the simulated data was generated by the LMM with single-component random effects, the second variance component should be zero. For the LRT test, the two nested models should be fitted using the same method, either REML or ML, and the fixed-effects design matrix, $X$, should be the same for the two models when using REML method.
