...
set.seed(2412)
dat <- simuRNAseq(counts, nGenes = 500, nCells = 120000, metadata = coldata, 
                  nsam = 25, ncls = 10, ntrt = 2, nDEgenes = 20, 
...
for (j in 1:ncol(p)){
  i <- (padj[, j] < 0.05)
  degenes <- rbind(degenes, data.frame(gene = rownames(p)[i], cluster = j, coef = beta[i, j], p = p[i, j]))
}
