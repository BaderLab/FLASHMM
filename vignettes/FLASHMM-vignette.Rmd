...
set.seed(2412)
dat <- simuRNAseq(counts, nGenes = 6000, nCells = 120000, metadata = coldata, 
                  nDEgenes = 280, minbeta = 0.5, maxbeta = 1, var.randomeffects = 0.1)
str(dat)
...
##We compute the log-transformed count matrix by sub-block matrices to reduce data size.
Y <- dat$counts
dat$counts <- NULL #Remove the counts.

ngrp <- 3 #number of sub-blocks
sizegrp <- round(nrow(Y)/ngrp)
sizegrp <- sizegrp + (ngrp*sizegrp < nrow(Y))
for (i in 1:ngrp){
...
##DE genes specific to a cell cluster with FDR < 0.05.
DEg <- NULL
for (j in 1:ncol(p)){
  i <- (padj[, j] < 0.05)
  DEg <- rbind(DEg, data.frame(gene = rownames(p)[i], cluster = j, coef = beta[i, j], p = p[i, j]))
}
rownames(DEg) <- NULL
dim(DEg)
head(DEg)
