...
#'
#' @examples
#' #Simulate multiple-samples (subjects) multiple-clusters (cell-types) scRNA-seq data.
#' set.seed(2412)
...
#'
#' @export
simuRNAseq <- function(counts, nGenes = nrow(counts), nCells = ncol(counts), metadata = NULL, samples.nested = TRUE, nsam = 25, ncls = 10, ntrt = 2, trt = NULL, nDEgenes = ncls, nDEgenesType, pDEgenesType = NULL, adjust.library.size = TRUE, direction = c("both", "up", "down"), minbeta = 0.25, maxbeta = 1, var.randomeffects = 0.01)
{
...
if (missing(counts)){
	...
	#counts <- exp(matrix(rnorm(nGenes*nCells, mean = 0, sd = 0.5), nrow = nGenes, ncol = nCells))
	mu <- 1 + rpois(nGenes, lambda = 1)
	counts <- matrix(rnbinom(nGenes*nCells, size = 1, mu = mu), nrow = nGenes, ncol = nCells)
...
if (is.null(metadata)){
	metadata <- data.frame(
	sam = paste0("S", sample.int(nsam, nCells, replace = TRUE)),
	cls = paste0("C", sample.int(ncls, nCells, replace = TRUE)),
	trt = LETTERS[sample.int(ntrt, nCells, replace = TRUE)])
	if (samples.nested) {
		#nsamtrt <- table(sample.int(ntrt, nsam, replace = TRUE))
		nsamtrt <- c(rmultinom(1, nsam, prob = rep(1/ntrt, ntrt)))
		trtset <- unique(metadata$trt)
		#metadata$sam <- as.vector(metadata$sam)
